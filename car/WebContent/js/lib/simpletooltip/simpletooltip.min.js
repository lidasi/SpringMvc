...
//动态加tip
function simpletooltipShow(obj, title, position){
	$(obj).addClass("simpletooltip");
	$(obj).attr("title", title);
	if (position) {
		$(obj).attr("data-simpletooltip-position", position);
	} else {
		$(obj).attr("data-simpletooltip-position", 'right');
	}
	tipInit();
	$(obj).mouseover();
}
...
//动态加tip,根据ID显示
function simpletooltipIdShow(objID, title, position){
	$(objID).addClass("simpletooltip");
	$(objID).attr("title", title);
	if (position) {
		$(objID).attr("data-simpletooltip-position", position);
	} else {
		$(objID).attr("data-simpletooltip-position", 'right');
	}
	tipInit();
	$(objID).mouseover();
}
...
//动态加tip,根据ID显示
function simpletooltipIdHide(objID){
	$(objID).unbind("mouseenter").unbind("mouseleave");//解除绑定hover
}
